...
const char * const BITCOIN_CONF_FILENAME = "upsidedoge.conf";
...
#else
    const char* pszModule = "upsidedoge";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UpsideDoge";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UpsideDoge";
#else
    // Unix
    return pathRet / ".upsidedoge";
#endif
#endif
}
