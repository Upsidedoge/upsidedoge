 ...
 */
static const char* BITCOIN_PID_FILENAME = "upsidedoged.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("upsidedoge-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/upsidedoge/upsidedoge>";
    const std::string URL_WEBSITE = "<https://definitelynotupsidedogetrader.com>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("upsidedoge-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if upsidedoge is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if upsidedoge is started while in a temporary directory.\n",
